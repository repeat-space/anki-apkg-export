...
const template = fs.readFileSync(
  __dirname + "/../templates/template.sql",
  "utf-8",
);
const now = Date.now();
...
test("Exporter.save", (t) => {
  ...
  t.truthy(
    zipFileSpy.calledWithMatch("collection.anki2"),
    "should save notes/cards db",
  );
  ...
  t.truthy(
    ["blob", "nodebuffer"].includes(zipGenerateAsyncSpy.args[0][0].type),
    "zip generates binary file",
  );
});
...
test("Exporter.addCard", (t) => {
  ...
  t.is(
    cardsUpdate[":nid"],
    notesUpdate[":id"],
    "should link both tables via the same note_id",
  );
});
...
test("Exporter.addCard with options (tags is array)", (t) => {
  ...
  t.deepEqual(
    notesTags,
    [""].concat(tags.map((tag) => tag.replace(/ /g, "_"))).concat([""]),
  );
});
